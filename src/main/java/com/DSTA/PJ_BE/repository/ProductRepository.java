...
public interface ProductRepository extends JpaRepository<Product, Long> {
    ...
    @Query(value = "SELECT p.id, p.name_product as name, p.slug_product as slug, p.information as information, p.summary as summary," +
    ...
    @Query(value = "SELECT p.name_product as name, p.slug_product as slug, p.information as information, p.summary as summary,"+
            "       p.description as description, p.image as image, p.stock as stock, p.price as price, p.discounted_price as discountedPrice,"+
            "       p.status as status, c.name_category as categoriesName, b.name_brand as brandsName, co.name_color as colorName, s.size_name as sizeName " +
            "FROM product p " +
            "JOIN categories c ON p.categories_id = c.id " +
            "JOIN brand b ON p.brands_id = b.id " +
            "JOIN product_color pc ON p.id = pc.product_id " +
            "JOIN color co ON pc.color_id = co.id " +
